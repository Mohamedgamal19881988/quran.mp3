...
async function getReciters() {
    ...
    const res = await fetch(`https://mp3quran.net/api/v3/reciters?language=${language}`)
...
async function getMoshaf(reciter) {
    ...
    const res = await fetch(`https://mp3quran.net/api/v3/reciters?language=${language}&reciter=${reciter}`)
    const data = await res.json()
    ...
    chooseMoshaf.innerHTML = `<option    
                            ...
                            data-surahList=""
                            >اختر مصحف</option>`
    moshafs.forEach(moshafs => {
        chooseMoshaf.innerHTML += `<option    
                            value="${moshafs.id}",
                            data-server="${moshafs.server}"
                            data-surahList="${moshafs.surah_list}"
                            >${moshafs.name}</option>`
    })
